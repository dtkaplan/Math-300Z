...
a. Yes, the flow runs directly from `x` to `y`.
b. Yes. The flow runs from `c` to each of `x` and `y`.
:::
...
a. Culture is an exogenous node; there are no incoming arrows to culture.
b. Yes.
c. From Culture there is a flow to outcome through expenditure. There is also a flow from Culture to Outcome via Participation.
:::
