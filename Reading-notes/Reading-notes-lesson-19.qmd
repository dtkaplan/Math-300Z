...
Gestation %>% 
  ...
  geom_jitter(alpha=0.2, width=0.2, height=0) +
  geom_violin(aes(group=parity), fill="blue", alpha=0.2, color=NA) +
  geom_boxplot(color="blue", fill=NA, alpha=.5)
```
...
`model_eval()` isn't doing the conversion to a response format. That's why I had to do the exponential transformations in the previous command.
