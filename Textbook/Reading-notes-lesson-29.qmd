...
The rule relating R^2^ to the number of coefficients associated is very simple (for the case where the explanatory variable has been shuffled): R^2^ will be random with mean value $\frac{k}{n-1}$. For the `shuffle(publisher)` model, the mean across many trials will be R^2^ = 158/324 = 0.49. 
...
@fig-amazon-book-shuffle suggests a way to distinguish between R^2^ resulting from the ISBN-effect and R^2^ that shows some genuine explanatory power: Check if R^2^ is substantially above the blue diagnonal line, that is if R^2^$\gg \frac{k}{n-1}$ where $k$ is the number of modelel coefficients.


## The F statistic

R^2^ must always be interpreted in the context of $k$ and $n$, looking to see if R^2^ is substantially above the blue line in @fig-amazon-book-shuffle. This is not difficult; the modeler always knows the size $n$ of the training data and can find $k$ as the number of coefficients in the model (not counting the Intercept term). The position of the blue line is also easily calculated for any model: $\frac{k}{n-1}$.
...
An alternative to F is widely used in some fields, particularly economics. Called "**adjusted R^2^**" or $R^2_\text{adj}$, visualize it by moving the calculated R^2^ downward and leftward, more-or-less in the direction of the blue line in @fig-amazon-book-shuffle. This movement adjusts an $R^2$ that lies on the blue line to $R^2_\text{adj} = 0$.

Debating the relative merits of using F or $R^2_\text{adj}$ can be left to the respective boosters of the disciplines that prefer one or the other. Before getting wrapped up in such debates, it is worth pointing out the $R^2_\text{adj}$ is just a rescaling of F.
...
```
Models with this structure, where all the explanatory variables in the smaller models also apply to the bigger models, are said to be "**nested**" in much the same way as for Russian dolls.
...
A good question, when adding explanatory variables to a model, is whether the new variable(s) add to the ability to account for the variability in the response variable. Since R^2^ never goes down when moving from a smaller to a larger model, we can't just check R^2^ on its own. A valuable technique called "**Analysis of Variance**," (ANOVA for short) looks at the incremental change in variance explained from a smaller model to a larger one, but puts this change in the context of the increase in $k$ from the small model to the large one. This increase-in-context can be converted to an F statistic. To illustrate:

```{r}
anova(model1, model2, model3, model4)
```

Focus on the F column of the report. The move from Model 1 to Model 2 produces F=57, clearly indicating that the `weight_oz` variable accounts for some of the list price. Moving from Model 2 to Model 3 creates a much less impressive F of 3.8. It is as if the added explanatory variable, `num_pages`, is just barely pulling its own "weight." Moving from Model 3 to Model 4 produces a below-threshold F of 1.3. In other words, in the context of `weight_oz` and `num_pages`, the `hard_paper` variable doesn't carry additional information about the list price. 

The last column of the report, labeled `Pr(>F)`, gives a translation of F into a universal 0 to 1 scale called a p-value. A large F produces a small p-value. The rule of thumb for reading p-values is that a value $p < 0.05$ indicates that the added variable brings new information about the response variable. We'll return to p-values and the controversy they have entailed in Lessons 36 through 38.
