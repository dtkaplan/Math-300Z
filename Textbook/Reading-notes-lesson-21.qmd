...
```{r echo=FALSE}
#| label: tab-tiny-dag01
#| tab-cap: "A `Tiny` ($n=10$) sample from `dag01`"
#| column: margin
Tiny
```
...
> Look at the 2-row sample (@tab-tiny-dag01) from the DAG and guess what the relationship might be. 
...
More data---a bigger sample---gives us a better shot at revealing the relationship hidden by the noise. @tab-small-dag01 shows a sample of size $n=10$: `r set.seed(103)`
...
```{r echo=FALSE}
#| label: tab-small-dag01
#| tab-cap: "A `Small` ($n=10$) sample from `dag01`"
#| column: margin
Small
```
