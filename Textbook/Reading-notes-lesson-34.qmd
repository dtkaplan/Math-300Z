...
To drive home an important point, I'm going to engage in a bit of metaphorical hyperbole. Imagine that all of the people who genuinely have condition C come from one planet, the Planet of the C. Similarly, the people with condition H come from another planet, the Planet of the H. (@fig-divided-by-condition)



```{r echo=FALSE}
#| label: fig-divided-by-condition
#| fig-cap: "The hypothetical planets of origin of the people in the classifier-building project. Everyone on the Planet of the C has condition C. (Note that all the glyphs are filled in.) Everyone on the Planet of the H has condition H. (The glyphs on planet H are hollow.)"
...
Suppose we perform the blood test on everybody on Planet of the C. A logician might point out that there is no need for a blood test, since everyone on Planet C has condition C. But our objective is to see how well the test performs. We'll measure the performance---on Planet C---by the fraction of inhabitants who get a + test.

This fraction applies only to Planet C and is called the "**sensitivity**" of the test. The fraction must be between zero and one, so it's tempting to call it a probability. **But don't.** We are going to give a new name to fractions computed on hypothetical planets: "**likelihoods**."

In everyday speech, "likelihood" is a synonym for "probability," but we are using words in a technical sense here. A likelihood aways comes from a hypothetical planet.

Another likelihood that's telling about how well the blood test performs is a fraction calculated on Planet H. This likelihood is called the test's "**specificity**" and is calculated as the fraction of - test results on Planet H.

As a more concise notation, let's write the sensitivity as ${\cal L}(+ | C)$. The plus reminds us that we're looking at the fraction of tests that are positive, the $C$ reminds us which hypothetical planet we are on. The ${\cal L}$ stands for "likelihood."

Similarly, we'll write the specificity as ${\cal L}(- | H)$. Sensitivity and specificity are both likelihoods, but there need not be any fixed relationship between them: They are calculated on different planets! To judge from the pictures in @fig-divided-by-condition, the sensitivity is about 90%---most of the glyphs are blue on Planet C. Similarly, the specificity is about 85%---most of the glyphs on Planet H are not blue.
