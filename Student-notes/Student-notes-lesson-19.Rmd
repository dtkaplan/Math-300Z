...
::: {.callout-warning}
## Make sure to update before deploying
...
Thus far in Math 300Z, we have used regression modeling and examined the coefficients created by training a model on data. 
...
**TASK**: Make a RESPEX graph suited to the model `thick ~ hard_paper`.
...
**TASK 1**: Using the next code chunk, set the `alpha=`, `color=`, and `fill=` to values of your preference, subject only to the constraint that the violin doesn't hide the data.

```{r warning=FALSE}
# for task 1
```

**TASK 2**: Compare your choices in Task 1 to those made in the readings in (Figure 19.3](https://dtkaplan.github.io/Math-300R/Textbook/Reading-notes-lesson-19.html#fig-violin-intro). What do you prefer about your choices and what do you prefer in Figure 19.3?

## Exercise 3

Common sense suggests that the thickness of a book depends on the number of pages. Let's investigate this numerically.

**TASK 1**: Make a RESPEX graph of `amazon_books` suitable for the model `thick ~ num_pages`.
...
**TASK 2**: Extract the `coefficients()` from `mod1`. 

```{r}
# for TASK 2
```

**TASK 3**: Based on the coefficients, say how thick a typical page is. (Remember the units!)
...
```
**TASK 1**: Explain in words how the above output for `num_pages=250 corresponds to the RESPEX graph of the model made just previously.
...
**TASK 2**: Add to the above chunk the wrangling commands to compute the *variance* of each of these three columns: the `.response` variable, the model `.output`, and the residuals `.resid`. (Hint: `summarize()` and `var()`.)

## Exercise 5

Let's explore the idea that a book's cover type---hard-cover or paperback---reflects the number of pages in the book. 

**TASK 1**: Use wrangling commands such as `group_by(hard_paper)` and `tally()` to figure out what fraction of the `amazon_books` are hard-cover? 

```{r}
# for Task 1
```

**TASK 2**: Use wrangling commands and `zero_one()`  to add a new variable `hardcover` to `amazon_books`. The value of the `hardcover` should be 1 when the book is hardcover. (Hints: You don't need `group_by()` for this task.)

```{r}
# for Task 2
```

**TASK 3**: Use wrangling commands to find the mean of the `hardcover` variable that you made in Task 2. 
