...
```{r}
# need to change this to use `model_eval()`: add a skeleton=TRUE argument
# When there is just 2 variables, one categorical one numeric, do 100 values for the numerical variable.
values <- lm(gestation ~ smoke, data = Gestation) %>% 
  mod_eval(nlevels=5, interval="confidence")
Gestation %>% ggplot(aes(x = smoke, y = gestation)) +
  geom_jitter(width=0.2, alpha=0.1) +
  geom_errorbar(data = values, aes(x=smoke, ymin=lower, ymax=upper), y=NA)
```
:::
