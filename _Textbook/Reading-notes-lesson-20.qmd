...
Models can be trained on the simulated data using the same techniques as for any other data. To illustrate, here we generate a sample of size $n=50$, then fit the model specification `c ~ a + b` and summarize by taking the coefficients. `r set.seed(101)`
