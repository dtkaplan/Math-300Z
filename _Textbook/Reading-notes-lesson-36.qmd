...
```{r}
lm(height ~ shuffle(mother), data=Galton) %>% coefficients()
```

Repeating the above generates another sample from Planet Null:

```{r}
lm(height ~ shufflree(mother), data=Galton) %>% coefficients()
```
...
The set of `mother` coefficients in these trials shows us the "sampling distribution under the Null." We can graph it:

```{r}
#| label: fig-shuffle-mother
#| fig-cap: "The sampling distribution of the `shuffle(mother)` coefficient. The red dot is the `mother` coefficient found from the sample collected on Planet Earth."
#| column: margin
Trials %>%
  filter(term == "shuffle(mother)") %>%
  ggplot(aes(y = coefficient, x = 1)) +
  geom_jitter(alpha = 0.5) +
  geom_violin(alpha = .2, fill = "blue", color = NA) +
  geom_point(aes(y = 0.31, x = 1), color="red")
```

The "sampling distribution under the Null" is represented by the gray dots. The violin gives an easier to read representation. For this model, the `mother` coefficient from the sample from Planet Earth is so far different that it cannot pretend to be an instance from Planet Null.
