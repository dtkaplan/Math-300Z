...
To illustrate, consider the model `gestation ~ parity`. In the next lines of code we build this model, training it with the `Gestation` data. Then we **evaluate** the model on the trained data. This amounts to using the model coefficients to generate a model output for each row in the training data, and can be accomplished with the `model_eval()` R function.
...
The output from `model_eval()` includes other columns of importance. For us, here, those are. the response variable itself (`gestation`) and the residuals from the model (`.resid`). There is a simple relationship between `gestation`, `.output` and `.resid`:

$$\mathtt{gestation} = \mathtt{.output} + \mathtt{.resid}\ .$$
...
```{r}
Evaluated %>%
  summarize(var_response = var(gestation),
...
```{r}
Evaluated %>%
  summarize(sd_response = sd(gestation),
            sd_output = sd(.output),
            sd_resid  = sd(.resid))
```
:::
