...
```{r echo=FALSE}
#| label: tbl-sampling-trials
#| tbl-cap: "`Trials` for seeing sampling variation."
### #| column: margin
Trials %>% head(25) 
```

`Trials` is a *sample of summaries*. (See [Lesson -@sec-lesson-20]). The row-to-row variation in`Trials` comes from sampling variation. We can *summarize* the variation in the *sample of summaries*. As always, our standard measure of variation is the standard deviation (or, equivalently, variance):
