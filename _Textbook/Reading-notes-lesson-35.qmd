# Accounting for prevalence {#sec-lesson-35}
...
@fig-divided-by-condition3 is drawn from the perspective of the epidemiologist or test developer. But it doesn't directly provide information of use to the patient, simply because the patient has only a test result ($\mathbb{P}$ or $\mathbb{H}$) but no definitive knowledge of the actual condition (C or H). 

Re-organizing the epidemiologist's graph can put it in a form relevant to the patient. Instead of plotting people by C or H, we can plot them by $\mathbb{P}$ or $\mathbb{H}$. This perspective is shown in @fig-divided-by-condition5, which is exactly the same people as in @fig-divided-by-condition2 but arranged differently.
...
For the patient who has gotten a $\mathbb{P}$ result, the left panel of @fig-divided-by-condition5 is highly informative. The patient can see that only a small fraction of the people testing $\mathbb{P}$ actually have condition C. (The people with C are shown as filled symbols.)
...
Similarly, the specificity is ${\cal L}_{\mathbb{H}}(H)$. But since our person got a $\mathbb{P}$ result, the likelihood ${\cal L}_{\mathbb{H}}(H)$ is not directly relevant. (It would be relevant only to a person with a $\mathbb{H}$ result.) Fortunately, there is a simple relationship between ${\cal L}_{\mathbb{P}}(H)$ and ${\cal L}_{\mathbb{H}}(H)$. If we know the probability of an H person getting a $\mathbb{H}$ result we can figure out the probability of an H person getting a $\mathbb{P}$ result.
...
Suppose that $p(C) = 1\%$ for this age of patient. (Consequently, $p(H) = 99\%$.) And imagine that the test taken by the patient has a threshold score of 1. From @fig-fraction-correct we can look up the sensitivity (${\cal L}_{\mathbb{P}}(C) = 0.95$) and specificity (${\cal L}_{\mathbb{P}}(H) = 0.50)$ for the test. Substituting these numerical values into Bayes' Rule gives

$$p(C {\ |\!\!|\  } \mathbb{P}) = \frac{0.95\times 0.01}{0.95\times 0.01 + 0.50*0.99} = 1.9\%$$
The $\mathbb{P}$ result has changed the probability that the patient has C from 1% to 1.9%. That's big proportionally, but not so big in absolute terms.
:::

The advantage of the Bayes' Rule form of the calculation over the $\mathbb{P}$ group in @fig-divided-by-condition4 is that it is very easy to do the Bayes' Rule calculation for any value of prevalence $p(C)$. Why would we be interested in doing this?
...
## Screening tests {#sec-screening-tests}
...
::: {.callout-note}
## Example: Breast cancer and mammography {#sec-bcm}

Let's look more closely at the details of breast-cancer screening. The reported sensitivity of digital mammography is 85% and the specificity is 90%.^[Diagnostic accuracy of digital versus film mammography: exploratory analysis of selected population subgroups in DMIST. Pisano ED, Hendrick RE, Yaffe MJ, et al. Radiology. 2008;246:376–383]

The [National Cancer Institute](https://knowyourchances.cancer.gov/special_cancer_tables.php) publishes cancer-risk tables. @fig-cancer-nci-risk shows the NCI table for breast cancer.

```{r echo=FALSE}
#| label: fig-cancer-nci-risk
knitr::include_graphics("www/breast-cancer-risk.png")
```
...
::: {.callout-warning}
## EXERCISE Bayes theorem in odds form:
