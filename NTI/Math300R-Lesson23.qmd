...
```{r}
lm(list_price ~ num_pages, data = amazon_books) %>% coefficients()
```
...
```{r}
trial <- function() {
  lm(list_price ~ num_pages, data = sample(amazon_books)) %>% 
    coefficients()
}
...
```{r}
do(3) * {lm(list_price ~ num_pages, data = amazon_books) %>% broom::tidy()}
```

```{r}
lm(list_price ~ num_pages, data = amazon_books) %>% broom::glance()
```
