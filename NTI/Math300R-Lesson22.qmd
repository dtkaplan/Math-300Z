...
```{r}
Sim <- dagsim(dag01, nrow=25)
lm(y ~ x, data = Sim) %>% coefficients()
```
...
```{r}
proc01 <- function() {
  lm(y ~ x, data = dagsim(dag01, nrow=25)) %>% coefficients()
}
proc01()
```
...
```{r}
One <- dagsim(dag01, nrow=25)
Two <- do(10) * {
  lm(y ~ x, data = dagsim(dag01, nrow=25)) %>%
    coefficients()
  }
Three <- Two %>% 
