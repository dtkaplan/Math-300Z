...
```{r echo=FALSE}
# Generate a seed that I can keep track of
# Sometimes an NA is generated
new_seed_from_time <- function() {
  res <- nanotime::nanotime(Sys.time()) %>% as.character() %>% substr(21,26) %>% as.integer()
  if (is.na(res)) res <- new_seed_from_time()
