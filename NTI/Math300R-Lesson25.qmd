...
To show "on average," we need to run many trials of the insample and many trials of the out-of-sample errors. Doing this requires considerable technical skill.


Here are two functions that do the calculation of the RMS error.

```{r}
trial <- function(n=100) {
  Samp <- sample(dag03, size=n)
  Mod <- lm(y ~ x, data = Samp)
...
```{r}
Expt <- do(1000)* trial(n = 4)
df_stats(~ out_rms, data = Expt, confint())
df_stats(~ in_rms, data = Expt, confint())
```

TO DO: PUT THESE INTO Narrow format so that you can plot out the distributions or use color.
