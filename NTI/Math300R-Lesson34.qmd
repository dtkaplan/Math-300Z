...
```{r}
Field_results <- sample(dag10, size=2000) %>% 
  mutate(prob = ifelse(y==1, 0.11, 1.00), rand = runif(nrow(.))) %>%
  filter(rand <= prob) %>% C1()
...
1. Given some classifier summaries, calculate the false-positive and false-negative rates as well as the sensitivity and specificity
